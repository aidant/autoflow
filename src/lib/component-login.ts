...
import type {
  UsernameAndPassword,
  UsernameAndPasswordCreateSchema,
  UsernameAndPasswordLoginSchema,
...
export const createAction =
  (
    ...
    schema: typeof UsernameAndPasswordCreateSchema | typeof UsernameAndPasswordLoginSchema,
  ) =>
  async ({ cookies, request }: RequestEvent) => {
    let user: User
    try {
      const form = formValidation(await request.formData(), schema)
      user = await db(form)
    } catch (error) {
